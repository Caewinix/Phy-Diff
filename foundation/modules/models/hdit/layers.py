...
class Denoiser(nn.Module):
    ...
    def loss(self, input, noise, sigma, **kwargs):
        c_skip, c_out, c_in = [things.append_dims(x, input.ndim) for x in self.get_scalings(sigma)]
        c_weight = self.weighting(sigma)
        noised_input = input + noise * things.append_dims(sigma, input.ndim)
...
class DenoiserWithVariance(Denoiser):
    def loss(self, input, noise, sigma, **kwargs):
        ...
        logvar = things.append_dims(logvar, model_output.ndim)
        target = (input - c_skip * noised_input) / c_out
